...
<div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
    <div class="bg-white rounded-2xl p-5 border border-slate-200 card-hover">
        ...
        <p class="text-2xl font-bold text-slate-800">{{ $stats['my_resources'] }}</p>
    ...
    <div class="bg-white rounded-2xl p-5 border border-slate-200 card-hover">
        ...
        <p class="text-2xl font-bold text-slate-800">{{ $stats['active_borrowing'] }}</p>
    ...
    <div class="bg-white rounded-2xl p-5 border border-slate-200 card-hover">
        ...
        <p class="text-2xl font-bold text-slate-800">{{ $stats['active_lending'] }}</p>
    ...
    <div class="bg-white rounded-2xl p-5 border border-slate-200 card-hover">
        ...
        <p class="text-2xl font-bold text-slate-800">{{ $stats['pending_requests'] }}</p>
...
<div class="bg-red-50 border border-red-200 rounded-2xl p-5 mb-8">
    ...
    <div class="space-y-2">
        ...
        <div class="flex items-center justify-between bg-white rounded-xl p-3">
            <span class="font-medium text-slate-700">{{ $item->resource->title }}</span>
            <span class="text-red-600 text-sm font-semibold">Due: {{ $item->due_date->format('M d, Y') }}</span>
        </div>
        @endforeach
    </div>
</div>
...
<div class="bg-white rounded-2xl border border-slate-200 p-6">
    <div class="flex items-center justify-between mb-5">
        ...
        <a href="{{ route('resources.index') }}" class="text-blue-600 text-sm font-semibold hover:underline">View All →</a>
    </div>
    @if($recentResources->count())
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        @foreach($recentResources as $resource)
        <a href="{{ route('resources.show', $resource) }}" class="block border border-slate-100 rounded-xl p-4 card-hover hover:border-blue-200">
            <div class="flex items-start gap-3">
                <div class="w-12 h-12 bg-blue-50 rounded-xl flex items-center justify-center flex-shrink-0">
                    @if($resource->photo)
                    <img src="{{ asset('storage/'.$resource->photo) }}" class="w-12 h-12 object-cover rounded-xl">
                    @else
                ...
                <div class="flex-1 min-w-0">
                    <p class="font-semibold text-slate-800 truncate">{{ $resource->title }}</p>
                    <p class="text-xs text-slate-500">{{ $resource->owner->name }}</p>
                    <span class="inline-block mt-1 text-xs px-2 py-0.5 rounded-full {{ $resource->type === 'free_lending' ? 'bg-green-100 text-green-700' : 'bg-orange-100 text-orange-700' }}">
    ...
    @else
    <p class="text-slate-400 text-center py-8">No resources available yet. <a href="{{ route('resources.create') }}" class="text-blue-600 font-semibold">Post one!</a></p>
    @endif
</div>
@endsection
